...
レスポンスから以下を読み取る：
- `weather` — 天気（Sunny / Partly cloudy / Overcast / Light rain / Rain / Snow / Fog 等）
- `temp_c` — 気温（℃）
- `local_time` — 現地時刻（フォーマット: "2026-06-06 14:30" など）

失敗した場合（`success: false` の場合）：ユーザーに天気と現在時刻を直接聞き、Step 3 へ進む。
